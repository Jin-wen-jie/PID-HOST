...
本文档面向下位机开发者，说明 PID-HOST 第一版需要 MCU 实现的 UART JSON Lines 协议。当前上位机支持两路电机 PID：`ch=0` 为电机1，`ch=1` 为电机2。
...
```text
{"type":"hello","seq":1}\n
{"type":"tel","ch":1,"t":123456,"sp":80.0,"pv":78.7,"out":32.1}\n
```
...
- `set_pid` 对 `ch=0` 和 `ch=1` 成功后都返回同 `seq` 的 `ack`。
- `set_sp` 对 `ch=0` 和 `ch=1` 成功后都返回同 `seq` 的 `ack`。
- `stream enabled=true` 后按 10-20 Hz 返回 `tel`。
- `stream enabled=false` 后停止返回 `tel`。
- 非法参数返回 `err`，错误码在表内。
- `tel.t` 单调递增。
- 两个通道的 `tel` 中 `sp/pv/out` 数值合理，且 `ch` 与对应电机一致。
